		...
		if(empty($_POST['txtNameSearch']) || empty($_POST['selSearchOption']))
			$msg = "Search option / name is empty!";
			 ...
			 if($_POST['selSearchOption'] == "Region"){
				$sqlSelUsers = mysql_query("select v.voter_id,v.firstname,v.lastname,v.address,v.region,v.phone,v.occupation,c.voter_id from voter v,candidates c where v.voter_id=c.voter_id and v.region='$searchName'") or die(mysql_error());
				...
				$sqlSelUsers = mysql_query("select v.voter_id,v.firstname,v.lastname,v.address,v.region,v.phone,v.occupation ,c.voter_id from voter v ,candidates c where c.voter_id=$searchName and v.voter_id=c.voter_id") or die(mysql_error());
				if(mysql_num_rows($sqlSelUsers) >= 1){
					$displayTitle .= '<table cellpadding="0" cellspacing="0" border="0" class="margintop_3">
									<tr style="background-color: #401; color: white;">
									<td class="border border_ts border_bs border_ls col_1">Id Number</td>
...
<div id="main_wrapper">
	...
	<div id="main_con">
		<?php if($_SESSION['user']=="Administrator")
			...
			<form name="search_user_form" method="post" action="candidatesearch.php">
