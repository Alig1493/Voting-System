<?php
	...
	$displayContent = "";
				...
				$sqlSelUsers = mysql_query("select v.voter_id,v.party,v.votedornot,e.firstname,e.region,e.phone from voted v,voter e where v.voter_id=e.voter_id ") or die(mysql_error());
				if(mysql_num_rows($sqlSelUsers) >= 1){
					$displayTitle .= '<table cellpadding="0" cellspacing="0" border="0" class="margintop_3">
									...
									<td class="border border_ts border_bs border_ls col_4">Phone</td>
									<td class="border border_ts border_bs border_ls border_rs col_1">Region</td></tr>';
					...
					if($voteornot==0){
						...
						<td style="font-size: 11px; border-bottom-style: double;">'.$phone.'</td>
						<td style="font-size: 11px; border-bottom-style: double;">'.$region.'</td></tr>';
						}
					}
